...
export default function Home() {
  ...
  return (
    <div className="bg-gray-100 min-h-screen flex flex-col justify-center items-center">
      ...
      {url && isImageAccepted === null && (
        <div className="mt-8">
          ...
          <div className="mt-4 flex gap-4">
            <button onClick={handleAccept} className="bg-green-500 text-white p-2 rounded hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500">
              Accept
            </button>
            <button onClick={handleReject} className="bg-red-500 text-white p-2 rounded hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500">
              Reject
            </button>
          </div>
        </div>
      ...
      {isImageAccepted === true && <p className="mt-8 text-green-500">Image accepted!</p>}
      {isImageAccepted === false && <p className="mt-8 text-red-500">Image rejected.</p>}
    </div>
  );
}
